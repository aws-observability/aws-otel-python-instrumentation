...
from amazon.opentelemetry.serviceevents.ast_transformation import install_ast_hooks
from amazon.opentelemetry.serviceevents.collectors.deployment_event_collector import DeploymentEventCollector
from amazon.opentelemetry.serviceevents.collectors.endpoint_collector import EndpointMetricCollector
...
LAMBDA_FUNCTION_NAME_CONFIG = "AWS_LAMBDA_FUNCTION_NAME"
...
def _build_log_otlp_exporter(logs_endpoint: str, headers: dict, compression):
    ...
    Returns an upstream ``OTLPLogExporter`` pointing at the collector-proxied
    endpoint. The ``x-aws-log-group`` / ``x-aws-log-stream`` headers travel with
    every batch (already populated in ``headers``).

    Imports are deferred so this module stays importable without the OTel SDK
    at import time.
    """
...
class ServiceEventsInstrumentation:
    ...
    def initialize(self) -> None:  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
        ...
        serviceevents_logger = logging.getLogger("amazon.opentelemetry.serviceevents")
        ...
        try:
            ...
            # pylint: disable=import-outside-toplevel
            from amazon.opentelemetry.serviceevents.python_monitor import get_sampling_mode, set_sampling_mode
            ...
            # Lazy import to avoid import-time coupling with python_monitor.
            # pylint: disable=import-outside-toplevel
            from amazon.opentelemetry.serviceevents.python_monitor import set_sampling_thresholds
    ...
    def _install_endpoint_span_processor(self, endpoint_collector, incident_snapshot_collector) -> bool:
        ...
        try:
            ...
            # pylint: disable=import-outside-toplevel
            from amazon.opentelemetry.serviceevents.processor.endpoint_span_processor import ServiceEventsSpanProcessor
    ...
    def _reinitialize_after_fork(self) -> None:
        ...
        try:
            # Lazy import to avoid import-time coupling with python_monitor.
            # pylint: disable=import-outside-toplevel
            from amazon.opentelemetry.serviceevents.python_monitor import reset_after_fork
...
def _is_lambda_environment() -> bool:
    return LAMBDA_FUNCTION_NAME_CONFIG in os.environ


def is_serviceevents_enabled() -> bool:
    ...
    return is_application_signals_enabled()


class ServiceEventsInstrumentor(BaseInstrumentor):
    """OpenTelemetry instrumentor entry point for ServiceEvents.

    Registered under the ``opentelemetry_instrumentor`` and
    ``opentelemetry_post_instrument`` entry-point groups. Delegates to the
    singleton :func:`get_serviceevents_instrumentation` /
    :class:`ServiceEventsInstrumentation` to perform the actual lifecycle work,
    so it carries no instrumentation logic of its own.
    """

    # Thin BaseInstrumentor entry point; methods delegate to the singleton and don't use self.
    ...
    def _instrument(self, **kwargs):
        ...
        try:
            ...
            if config is None:
                resource_attributes = (
                    ResourceAttributes.from_otel_resource(resource) if resource else ResourceAttributes()
                )
            ...
            as_enabled = is_application_signals_enabled()
            ...
            if instrumentation is None:
                logger.warning("Failed to get ServiceEvents instrumentation instance")
                return

            instrumentation.initialize()
            logger.info("ServiceEvents instrumentation initialized")
        except Exception as exc:  # pylint: disable=broad-exception-caught  # telemetry must not crash app
            logger.error("Failed to initialize ServiceEvents: %s", exc, exc_info=True)

    def _uninstrument(self, **kwargs):
        instrumentation = get_serviceevents_instrumentation()
