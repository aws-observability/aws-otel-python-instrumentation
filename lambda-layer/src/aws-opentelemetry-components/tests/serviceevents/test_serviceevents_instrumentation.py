...
class TestGetServiceEventsInstrumentation(TestCase):
    ...
    def test_returns_none_when_no_instance_and_no_config(self):
        ...
        from amazon.opentelemetry.serviceevents.serviceevents_instrumentation import get_serviceevents_instrumentation
...
class TestServiceEventsOutputFileMode(TestCase):
    ...
    def setUp(self):
        ...
        hooks_patcher = patch("amazon.opentelemetry.serviceevents.serviceevents_instrumentation.install_ast_hooks")
        hooks_patcher.start()
...
class TestEndpointMeasurementMode(TestCase):
    ...
    def test_install_span_processor_is_idempotent_on_same_provider(self):
        ...
        with patch("opentelemetry.trace.get_tracer_provider", return_value=provider):
            self.assertTrue(inst._install_endpoint_span_processor(MagicMock(), MagicMock()))
            # Second call (re-init / forked child inheriting the provider) is a no-op add.
            self.assertTrue(inst._install_endpoint_span_processor(MagicMock(), MagicMock()))
    ...
    def test_provider_has_endpoint_processor_handles_unexpected_shape(self):
