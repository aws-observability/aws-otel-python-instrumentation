...
class TestBuildLogOtlpExporter(TestCase):
    ...
    Direct-to-CloudWatch SigV4 export was removed when ServiceEvents was extracted into
    ``aws-opentelemetry-components``; ServiceEvents now exports exclusively through the
    collector-proxied OTLP endpoint, so every endpoint yields the plain upstream exporter.
    """
