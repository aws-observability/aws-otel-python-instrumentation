...
_MONITOR_PATH = "amazon.opentelemetry.serviceevents.processor.endpoint_span_processor._ServiceEventsMonitorState"
...
class TestOnEnd(TestCase):
    ...
    def _run(self, span):
        with patch(_MONITOR_PATH) as monitor_cls, patch(
            "amazon.opentelemetry.serviceevents.processor.endpoint_span_processor._extract_error_from_call_path"
        ) as extract_mock:
    ...
    def test_context_cleared_in_finally(self):
        ...
        with patch(_MONITOR_PATH) as monitor_cls, patch(
            "amazon.opentelemetry.serviceevents.processor.endpoint_span_processor.clear_current_operation"
        ) as clear_op, patch(
            "amazon.opentelemetry.serviceevents.processor.endpoint_span_processor._extract_error_from_call_path"
        ) as extract_mock:
            extract_mock.return_value = None
