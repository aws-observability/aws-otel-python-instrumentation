...
class TestAwsMetricAttributesSpanProcessor(TestCase):
    def setUp(self):
        self.generator_mock: _AwsMetricAttributeGenerator = MagicMock()
        self.test_resource: Resource = Resource.get_empty()
        self.processor: AwsMetricAttributesSpanProcessor = AwsMetricAttributesSpanProcessor(
            self.test_resource, self.generator_mock
        )
    ...
    def test_export_delegation_without_attributes_or_modification(self):
        ...
        self._configure_mock_for_on_end(span_data_mock, metric_attributes)

        self.processor.on_end(span_data_mock)
    ...
    def test_export_delegation_without_attributes_but_with_modification(self):
        ...
        self.processor.on_end(span_data_mock)
        exported_attributes: Attributes = span_data_mock._attributes
        self.assertEqual(len(exported_attributes), len(metric_attributes))
        for key, value in metric_attributes.items():
            self.assertEqual(span_data_mock._attributes[key], value)
    ...
    def test_on_end_with_multiple_spans(self):
        ...
        self._configure_mock_for_on_end_with_multiple_side_effect(
            [span_data_mock1, span_data_mock2, span_data_mock3],
            [metric_attributes1, metric_attributes2, metric_attributes3],
        )

        self.processor.on_end(span_data_mock1)
        self.processor.on_end(span_data_mock2)
        self.processor.on_end(span_data_mock3)

        self.assertEqual(len(span_data_mock2._attributes), len(metric_attributes2) + len(span_attributes2))
        for key, value in metric_attributes2.items():
            self.assertEqual(span_data_mock2._attributes[key], value)
    ...
    def test_overridden_attributes(self):
        ...
        self.processor.on_end(span_data_mock)
        self.assertEqual(span_data_mock._attributes["key1"], "new value1")
        self.assertEqual(span_data_mock._attributes["key2"], "old value2")
        self.assertEqual(span_data_mock._attributes["key3"], "new value3")
    ...
    def test_export_delegation_with_two_metrics(self):
        ...
        expected_attribute_count: int = sum(len(attrs) for attrs in attribute_map.values()) + len(span_attributes)
        self.assertEqual(len(span_data_mock._attributes), expected_attribute_count)
        ...
        for key, value in dependency_metric.items():
            if key == AWS_SPAN_KIND:
                self.assertNotEqual(span_data_mock._attributes[key], value)
            else:
                self.assertEqual(span_data_mock._attributes[key], value)

        self.assertEqual(span_data_mock._attributes[AWS_SPAN_KIND], LOCAL_ROOT)
    ...
    def test_on_end_with_dependency_metrics(self):
        span_attributes: Attributes = self._build_span_attributes(_CONTAINS_ATTRIBUTES)
        span_data_mock: ReadableSpan = MagicMock()
    ...
    def _configure_mock_for_on_end(self, span_data_mock: ReadableSpan, metric_attributes: Attributes):
    ...
    def _configure_mock_for_on_end_with_multiple_side_effect(
