...
class TestSamplingRuleApplier(TestCase):
    ...
    def test_applier_with_boost(self):
        sampling_rule = _SamplingRule(
            ...
            URLPath="*",
            Version=1,
            SamplingRateBoost=_SamplingRateBoost(MaxRate=0.9, CooldownWindowMinutes=5),
        )
        ...
        rule_applier = rule_applier.with_target(
            _SamplingTarget(
                ...
                ReservoirQuota=0,
                ReservoirQuotaTTL=0,
                SamplingBoost=_SamplingBoost(
                    BoostRate=1.0, BoostRateTTL=(mock_clock.now() + datetime.timedelta(seconds=20)).timestamp()
                ),
                RuleName="test",
            )
        )
