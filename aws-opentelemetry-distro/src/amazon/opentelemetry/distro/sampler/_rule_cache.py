...
class _RuleCache:
    ...
    def should_sample(
        ...
        upstream_matched_rule = parent_span_context.trace_state.get(
        ...
        for rule_applier in self.__rule_appliers:
            if rule_applier.matches(self.__resource, attributes):
                ...
                rule_to_propagate: Optional[str]
                if upstream_matched_rule is not None:
                    rule_to_propagate = self._hash_to_rule_map.get(upstream_matched_rule, None)
                elif parent_span_context.is_valid:
                    rule_to_propagate = None
                else:
                    rule_to_propagate = rule_applier.sampling_rule.RuleName
                hashed_rule = self._rule_to_hash_map.get(rule_to_propagate, upstream_matched_rule)
                break

        if result is not None:
            return _AwsSamplingResult(result.decision, result.attributes, result.trace_state, hashed_rule)
    ...
    def adapt_sampling(self, span: ReadableSpan, span_batcher: Callable[[ReadableSpan], None]) -> None:
        ...
        if should_boost_sampling or is_new_trace:
            ...
            for applier in self.__rule_appliers:
                ...
                if applier.matches(self.__resource, span.attributes):
    ...
    def __is_anomaly(self, span: ReadableSpan) -> "_AnomalyDetectionResult":
        ...
        status_code: int = span.attributes.get(HTTP_RESPONSE_STATUS_CODE)
        ...
        if anomaly_conditions:
            ...
            for condition in anomaly_conditions:
                ...
                operations_to_check = condition.operations
                if operations_to_check is not None and operation not in operations_to_check:
                    continue
