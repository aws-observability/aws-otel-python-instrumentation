...
# ServiceEvents was extracted into the standalone ``aws-opentelemetry-components`` distribution;
# the span-name/operation helpers below moved there. The distro depends on that package and imports
# them back so this module stays the single import surface the App Signals processors already use
# (attribute_propagating_span_processor*, _aws_metric_attribute_generator, sampler/_rule_cache import
# these names from here unchanged).
# pylint: disable=unused-import
from amazon.opentelemetry.serviceevents.span_processing_util import (  # noqa: F401  (re-exported)
    INTERNAL_OPERATION,
    UNKNOWN_OPERATION,
...
# UNKNOWN_OPERATION and INTERNAL_OPERATION are re-exported from serviceevents.span_processing_util above.
