...
"""Bytecode injection engine for Python 3.9-3.11."""
...
class InjectionState:
    """Bytecode injection state for restoration."""
...
class BytecodeInjectionEngine(InstrumentationEngine):
    ...
    def enable_breakpoints_for_function(
        ...
        """Inject line-level breakpoints into a function's bytecode."""
    ...
    def disable_breakpoints_for_function(self, code: CodeType, func: FunctionType) -> None:
        """Disable all breakpoints for a function and restore original bytecode."""
        if not self._initialized:
            logger.warning("Engine not initialized")
            return
        ...
        try:
            ...
            with self._lock:
                ...
                # Skip popping handlers if a sibling in the same __globals__
                # is still instrumented — would NameError inside user code.
    ...
    def _breakpoint_handler(  # pylint: disable=too-many-locals
        ...
        """Line-breakpoint callback invoked from injected bytecode."""
        try:
            ...
            # capture_locals: None=skip, []=all, [...]=named
    ...
    def _create_code_with_breakpoints(
        ...
        """Inject line-breakpoint calls before each requested line."""
        try:
            bc = Bytecode.from_code(code)
            new_instructions = []
            injected_lines = set()
            ...
            if missing_lines:
                logger.warning(
                    "Could not inject breakpoints at lines %s in %s",
    ...
    def cleanup(self) -> None:
        """Restore all modified functions and clear engine state."""
        try:
            if not self._initialized:
                return
            ...
            with self._lock:
                ...
                logger.debug(
                    "BytecodeInjectionEngine cleaned up: %d restored, %d failed",
                    restored_count,
                    failed_count,
                )
