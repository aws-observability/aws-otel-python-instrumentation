...
_UDP_PORT = 2000
_HTTP_PORT = 8080
_BIND_HOST = os.getenv("MOCK_COLLECTOR_BIND_HOST")
...
def _extract_value(any_value):
    """Extract a typed value from a protobuf AnyValue."""
    ...
    if which == "array_value":
        return [_extract_value(v) for v in any_value.array_value.values]
    return str(any_value)


def _udp_listener():
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind((_BIND_HOST, _UDP_PORT))  # nosec - test container on isolated Docker network
        ...
        try:
            ...
            with lock:
                ...
                for rs in req.resource_spans:
                    resource_attrs = {kv.key: _extract_value(kv.value) for kv in rs.resource.attributes}
                    for ss in rs.scope_spans:
                        ...
                        for span in ss.spans:
                            span_data = {
                                ...
                                "attributes": {kv.key: _extract_value(kv.value) for kv in span.attributes},
                                "events": [{"name": e.name, "timestamp": e.time_unix_nano} for e in span.events],
...
if __name__ == "__main__":
    ...
    server = HTTPServer((_BIND_HOST, _HTTP_PORT), QueryHandler)
